...
public void setup() {background(0);imagein();
  parseFile();//obj
  /*parseFile2();*/ //mtl
}
   
public void draw() {if (oneloop1==0){setupcheck();oneloop1=1;}windowcheck();}
...
public void windowcheck(){
  ...
  if (windowup==1){//preview
    ...
    stroke(0xffFF05FC);fill(0xffFF05FC); rect(250, 20, 90, 20);fill(0);stroke(0);textSize(20);text("retrieve", 259, 38);strokeWeight(5);
    ...
    if(ex==3){triangle_3D(-200,0,0,0,200,0,0,0,150);tri_gappoint(-200,0,0,0,200,0,0,0,150,0.1f);}
    if(ex==4){show();}
    if(ex==5){ex5();}
    if(ex==6){ex6();}
  ...
  fill(0xff00FCD0);stroke(0xff00FCD0);rect(150,200,600,300);fill(0);stroke(0);
  fill(0xffF8FC00);stroke(0xffF8FC00);rect(800,200,600,300);fill(0);stroke(0);
  ...
  strokeWeight(1);fill(0xffFA08E2);stroke(0xffFA08E2);rect(150, 20, 150, 40); fill(0);stroke(0); 
  ...
  showstring2(160, 75);
  fill(0xffFA08E2);stroke(0xffFA08E2);rect(250, 20, 90, 20);fill(0);stroke(0); 
  textSize(20);text("console", 260, 38);
  stroke(0xff0091FC);fill(0xff0091FC);rect(650, 20, 90, 20);fill(0);stroke(0);
...
public float B(int i, float t){if(i==0) {return (float)(1-t)*(1-t);}else if(i==1) {return 2*t*(1-t);}else {return t*t;}}
public float Bt(int i,float t){if(i==0) {return 2*(t-1);}else if(i==1) {return 2*(1-2*t);}else {return 2*t;}}
...
public void com(float zmin,float zmax,float anglemin,float anglemax){
  ...
  float dismax1=0.1f,dismax2=0.1f,dismax3=0.1f,dismax4=0.1f;
  ...
  for(int j=1;j<=4;j++){float findz=(zmax-zmin)/5*j+zmin;
    ...
    rayshow[0][0]=ray[0];rayshow[0][1]=ray[1];rayshow[0][2]=ray[2];rayshow[1][0]=ray[3];rayshow[1][1]=ray[4];rayshow[1][2]=ray[5];
    checkcirto3D(r3D[0],r3D[1],r3D[2]);
    if(Acir[1]-radians(anglemax%360)<=0.01f && Acir[1]-radians(anglemax%360)>=-0.01f){continue;}if(Acir[0]<=5){continue;}
    show[0][showend][0]=r3D[0];show[0][showend][1]=r3D[1];show[0][showend][2]=r3D[2];vsh[showend]=0;showend++;
    float pax,pay,paz,dx,dy,dz,ansx,ansy,ansz;
  ...
  for(int i=0;i<N;i++){p[i]=new vector(Bstrange.stackArr[i][0],Bstrange.stackArr[i][1],Bstrange.stackArr[i][2]);u[i]=0.5f;v[i]=0.5f;}
  ...
  win5out[win5outlength]="beziersurfacelinerc33("+Float.toString(255)+","+Float.toString(0)+","+Float.toString(255)+",0.0,"+Float.toString(0.05f)+")";       win5outlength++;return; }
  Rearrange();}
  ...
  Ast_Bezier_surface_line_rc33(0.05f);
  ...
  win5out[win5outlength]="checkrc33(2,"+Float.toString(b[2][0].vectori(0))+","+Float.toString(b[2][0].vectori(1))+","+Float.toString(b[2][0].vectori(2))+","+Float.toString(b[2][1].vectori(0))+","+Float.toString(b[2][1].vectori(1))+","+
  Float.toString(b[2][1].vectori(2))+","+Float.toString(b[2][2].vectori(0))+","+Float.toString(b[2][2].vectori(1))+","+Float.toString(b[2][2].vectori(2))+")";win5outlength++;
  win5out[win5outlength]="beziersurfacelinerc33("+Float.toString(255)+","+Float.toString(0)+","+Float.toString(255)+",0.0,"+Float.toString(0.05f)+")";win5outlength++;}
}

public void rangein(float zmin,float zmax,float anglemin,float anglemax){
...
public void Find_b11(){
   vector r= new vector(); float s=0.01f;
   for(int i=0; i<N; i++){
      if(norm(BSu(u[i], v[i]))<0.001f || norm(BSv(u[i], v[i]))<0.001f) continue;
      vector y= new vector();
...
public void Ast_Bezier_surface_line_rc33(float gap){
  int i,j;float u,v;float[][][] save=new float[1005][1005][3];
   for(u=0;u<=1.001f;u+=gap){for(v=0;v<=1.001f;v+=gap){float sumx=0,sumy=0,sumz=0;
...
public float floattry(String str){
...
public void cylinder_ray(float x,float y, float zmin,float zmax, float r,float findz,float angle){
...
public void ray_triangle(float x,float y, float zmin,float zmax, float r,float findz,float angle,float trix1,float triy1,float triz1,float trix2,float triy2,float triz2,float trix3,float triy3,float triz3){
...
public void tri_gappoint(float trix1,float triy1,float triz1,float trix2,float triy2,float triz2,float trix3,float triy3,float triz3,float gap){
  float x,y,z;
...
public void triangle_3D(float trix1,float triy1,float triz1,float trix2,float triy2,float triz2,float trix3,float triy3,float triz3){
...
public void cylinder(float x,float y, float zmin,float zmax, float r){
   float z,w,m,n,l,p;
...
public void checkcirto3D(float x,float y,float z){
   Acir[0]=sqrt(x*x+y*y);
...
public void rebar() {
  ...
  if ((mouseX>=400)&&(mouseY>=0)&&(mouseY<=800)&&(mouseX<=1500)) {
    ...
    fill(255, 0, 0);ellipse((mouseY/800.0f)*300+50, 83, 15, 15);
    fill(0);rect(50-10, 145-10, 300+20, 20);
    fill(100);rect(50, 145-5, 300, 10);
    fill(255, 0, 0);ellipse(((mouseX-400)/1100.0f)*300+50, 145, 15, 15);
    fill(0);rect(50-10, 200-10, 300+20, 20);
    fill(100);rect(50, 200-5, 300, 10);
    fill(255, 0, 0);ellipse(size/1.5f*300+50, 200, 15, 15);} 
  else {
    fill(0);rect(50-10, 83-10, 300+20, 20);
    fill(100);rect(50, 83-5, 300, 10);
    ...
    fill(0);rect(50-10, 145-10, 300+20, 20);
    fill(100);rect(50, 145-5, 300, 10);
    fill(255, 0, 0);ellipse(ceta/360.0f*300+50, 145, 15, 15);
    fill(0);rect(50-10, 200-10, 300+20, 20);
    fill(100);rect(50, 200-5, 300, 10);
    fill(255, 0, 0);ellipse(size/1.5f*300+50, 200, 15, 15);
  }}
  
public void make_bar() {
  bar1(83, 50, 350);
  bar2(145, 50, 350);
  bar3(200, 50, 350);
  bar4(50, 300, 450);
...
public void bar1(float y, float x1, float x2) {    //pi
  pi_bary=y;
...
public void bar2(float y, float x1, float x2) {    //ceta
  ceta_bary=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
...
public void bar3(float y, float x1, float x2) {    //size
  size_bary=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
  fill(255, 0, 0);ellipse(size_barx, size_bary, 15, 15);
  if ((mouseButton==LEFT)&&(mouseX<=x2)&&(mouseX>=x1)&&(mouseY>y-10)&&(mouseY<y+10)) {
      size_barx=mouseX;size=(size_barx-x1)/(x2-x1)*1.5f;}
  strokeWeight(5);}
  
public void bar4(float x, float y1, float y2) {    //RMatrial
  RMatrialx=x;
...
public void bar5(float x, float y1, float y2) {    //GMaterial
  GMatrialx=x;
  fill(0);rect(x-10, y1-10, 20, y2-y1+20);
  fill(100);rect(x-5, y1, 10, y2-y1);
...
public void bar6(float x, float y1, float y2) {    //BMaterial
  BMatrialx=x;
  fill(0);rect(x-10, y1-10, 20, y2-y1+20);
  fill(100);rect(x-5, y1, 10, y2-y1);
...
public void bar7(float x, float y1, float y2) {   //lightx
  lightxx=x;
  fill(0);rect(x-10, y1-10, 20, y2-y1+20);
  fill(100);rect(x-5, y1, 10, y2-y1);
...
public void bar8(float x, float y1, float y2) {   //lighty
  lightyx=x;
  fill(0);rect(x-10, y1-10, 20, y2-y1+20);
  fill(100);rect(x-5, y1, 10, y2-y1);
...
public void bar9(float x, float y1, float y2) {   //lightz
  lightzx=x;
  fill(0);rect(x-10, y1-10, 20, y2-y1+20);
  fill(100);rect(x-5, y1, 10, y2-y1);
...
public void bar10(float y, float x1, float x2) {   //lightcolorR
  lightcolorRy=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
...
public void bar11(float y, float x1, float x2) {   //lightcolorG
  lightcolorGy=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
...
public void bar12(float y, float x1, float x2) {   //lightcolorB
  lightcolorBy=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
...
public void bar13(float y, float x1, float x2) {   //lightpower
  lightpowery=y;
  fill(0);rect(x1-10, y-10, x2-x1+20, 20);
  fill(100);rect(x1, y-5, x2-x1, 10);
...
public void madeswitch1(float x, float y, int R1, int G1, int B1, int R2, int G2, int B2, float dx, float dy) {
  strokeWeight(5);
...
public void madeswitch2(float x, float y, int R1, int G1, int B1, int R2, int G2, int B2, float dx, float dy) {
  strokeWeight(5);
  fill(255);rect(x, y, dx, dy);
  fill(0);line(x+dx/2, y, x+dx/2, y+dy);
...
public void setangle() {
  background(255-255*coordinateBW);fill(0xffFFEA00);rect(0, 0, 400, 800);
  fill(255*coordinateBW);rect(370, 0, 25, 800);fill(0);
  if ((mouseX>=400)&&(mouseY>=0)&&(mouseY<=800)&&(mouseX<=1500)) {
    ceta=(mouseX-400)/1100.0f*360.0f;pi=mouseY/800.0f*360.0f;}
  ...
  stroke(0xff0091FC);fill(0xff0091FC); rect(250, 20, 90, 20);fill(0);stroke(0);textSize(20);text("retrieve", 259, 38);strokeWeight(5);
...
public void mouseWheel(MouseEvent event) {
  if (windowup==1){
  if (((mouseX>=400)&&(mouseY>=0)&&(mouseY<=800)&&(mouseX<=1500))&&(size>=0.1f)&&(size<=1.49f)) {
      float e = event.getCount()*0.01f;size-=e;setangle();makecoordinate();}
      strokeWeight(5);fill(0);make_bar();rebar();}}
public void Bezier3Ddata(float x0,float y0,float z0,float x1,float y1,float z1,float x2,float y2,float z2, float num,float siz){if(siz==0){siz=size;}
  ...
  float z,w,k,t=0;
  for(t=0;t<=1;t+=0.005f){z=t*t*x0+2*t*(1-t)*x1+(1-t)*(1-t)*x2;w=t*t*y0+2*t*(1-t)*y1+(1-t)*(1-t)*y2;k=t*t*z0+2*t*(1-t)*z1+(1-t)*(1-t)*z2;point3D_RGBdata(z,w,k,255,0,0,5,siz);Bpoint3D(z,w,k);}
...
public void Bezier_surface_control_line_rc44data(float R,float G,float B,float siz){if(siz==0){siz=size;}
...
public void Bezier_surface_control_line_rc33data(float R,float G,float B,float siz){if(siz==0){siz=size;}
...
public void Bezier_surface_line_rc44data(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
...
public void Bezier_surface_line_rc33data(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
  ...
  int i,j;float u,v,max=0;float[][][] save=new float[1005][1005][3];
   for(u=0;u<=1.001f;u+=gap){for(v=0;v<=1.001f;v+=gap){float sumx=0,sumy=0,sumz=0;
      ...
      if(max*max<sumx*sumx+sumy*sumy+sumz*sumz){max=sqrt(sumx*sumx+sumy*sumy+sumz*sumz);}}}
      data[count]="#maxdistance: "+Float.toString(max);count++;
   for(u=0;u<=1.001f;u+=gap){float savx=0,savy=0,savz=0;
    for(v=0;v<=1.001f;v+=gap){float k,sumx=save[(int)(u*1000)][(int)(v*1000)][0],sumy=save[(int)(u*1000)][(int)(v*1000)][1],sumz=save[(int)(u*1000)][(int)(v*1000)][2];
      k=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max;
      point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k)*k))+50,(int)(G*sqrt(sqrt(k)*k))+50,(int)(B*sqrt(sqrt(k)*k))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(v==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
   ...
   for(v=0;v<=1.001f;v+=gap){float savx=0,savy=0,savz=0;
    for(u=0;u<=1.001f;u+=gap){float k,sumx=save[(int)(u*1000)][(int)(v*1000)][0],sumy=save[(int)(u*1000)][(int)(v*1000)][1],sumz=save[(int)(u*1000)][(int)(v*1000)][2];
      k=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max;
      if(u==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3Ddata(savx,savy,savz,sumx,sumy,sumz,siz,(int)(R*sqrt(sqrt(k)*k))+50,(int)(G*sqrt(sqrt(k)*k))+50,(int)(B*sqrt(sqrt(k)*k))+50,3);Bline3D(savx,savy,savz,sumx,sumy,sumz);
        savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}}
        
public void Bezier_triangle_control_line44data(float R,float G,float B,float siz){int i,j,k;if(siz==0){siz=size;}
...
public void Bezier_triangle_line44data(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
...
public void Bezier_triangle_control_line33data(float R,float G,float B,float siz){int i,j,k;if(siz==0){siz=size;}
...
public void Bezier_triangle_line33data(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
  ...
  for(s=0;s<=1;s+=gap){for(t=0;t<=1;t+=gap){if(s+t>1){continue;}u=1.01f-s-t;float sumx=0,sumy=0,sumz=0;
      ...
      if(max*max<sumx*sumx+sumy*sumy+sumz*sumz){max=sqrt(sumx*sumx+sumy*sumy+sumz*sumz);}}}
      data[count]="#maxdistance: "+Float.toString(max);count++; 
  for(s=0;s<=1;s+=gap){float savx=0,savy=0,savz=0;for(t=0;t<=1;t+=gap){if(s+t>1){continue;}u=1.01f-s-t;
      ...
      Bline3D(savx,savy,savz,sumx,sumy,sumz);savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
  for(t=0;t<=1;t+=gap){float savx=0,savy=0,savz=0;for(u=0;u<=1;u+=gap){if(u+t>1){continue;}s=1.01f-u-t;
        float k1,sumx=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][0],sumy=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][1],sumz=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][2];
        k1=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max; point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(u==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      ...
      Bline3D(savx,savy,savz,sumx,sumy,sumz);savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
  for(u=0;u<=1;u+=gap){float savx=0,savy=0,savz=0;for(s=0;s<=1;s+=gap){if(s+u>1){continue;}t=1.01f-u-s;
        float k1,sumx=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][0],sumy=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][1],sumz=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][2];
        k1=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max; point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(s==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3Ddata(savx,savy,savz,sumx,sumy,sumz,siz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,3);
      Bline3D(savx,savy,savz,sumx,sumy,sumz);savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
  for(t=0;t<=1;t+=gap){float savx=0,savy=0,savz=0;for(s=0;s<=1;s+=gap){if(s+t>1){continue;}u=1.01f-s-t;
        float k1,sumx=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][0],sumy=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][1],sumz=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][2];
        k1=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max;point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(s==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3Ddata(savx,savy,savz,sumx,sumy,sumz,siz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,3);
      Bline3D(savx,savy,savz,sumx,sumy,sumz);savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
  for(u=0;u<=1;u+=gap){float savx=0,savy=0,savz=0;for(t=0;t<=1;t+=gap){if(u+t>1){continue;}s=1.01f-u-t; 
        float k1,sumx=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][0],sumy=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][1],sumz=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][2];
        k1=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max;point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(t==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3Ddata(savx,savy,savz,sumx,sumy,sumz,siz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,3);
      Bline3D(savx,savy,savz,sumx,sumy,sumz);savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
  for(s=0;s<=1;s+=gap){float savx=0,savy=0,savz=0;for(u=0;u<=1;u+=gap){if(s+u>1){continue;}t=1.01f-u-s;
        float k1,sumx=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][0],sumy=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][1],sumz=save2[(int)(s*10)*10][(int)(t*10)*10][(int)(u*10)*10][2];
        k1=sqrt(sumx*sumx+sumy*sumy+sumz*sumz)/max;point3D_RGBdata(sumx,sumy,sumz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,5,siz);Bpoint3D(sumx,sumy,sumz);
      if(u==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3Ddata(savx,savy,savz,sumx,sumy,sumz,siz,(int)(R*sqrt(sqrt(k1)*k1))+50,(int)(G*sqrt(sqrt(k1)*k1))+50,(int)(B*sqrt(sqrt(k1)*k1))+50,3);
...
public void example_Bezier_rectdata(){ 
  ...
  Bezier_surface_line_rc33data(255,0,0,size,0.2f);Bezier_surface_control_line_rc33data(237,0,255,size);}
  
public void example_Bezier_tridata(){
  data[count]="#function name: example_Bezier_rectdata 0 sources";count++;
  checktr33_sampledata();
  Bezier_triangle_line33data(255,0,0,size,0.2f);Bezier_triangle_control_line33data(237,0,255,size);}
public void example_Bezier_linedata(){data[count]="#function name: example_Bezier_linedata 0 sources";count++;Bezier3Ddata(100,100,100,-100,-100,-100,50,200,-150,1,0);}

public void example_Bezier_rect44data(){ 
  ...
  Bezier_surface_line_rc44data(255,0,0,size,0.05f);Bezier_surface_control_line_rc44data(255,246,142,size);}

public void example_Bezier_rect33data(){ 
  ...
  Bezier_surface_line_rc33data(255,0,0,size,0.05f);Bezier_surface_control_line_rc33data(255,246,142,size);}

public void example_Bezier_tri44data(){
  data[count]="#function name: example_Bezier_tri44data 0 sources";count++;checktr44data();
  Bezier_triangle_line44data(255,0,0,size,0.1f);Bezier_triangle_control_line44data(255,246,142,size);}

public void example_Bezier_tri33data(){ data[count]="function name: example_Bezier_tri33data 0 sources";count++;checktr33data();
  Bezier_triangle_line33data(0,255,0,size,0.2f);Bezier_triangle_control_line33data(255,246,142,size);}

public void checkrc44data(int m,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3,float x4,float y4,float z4){   
...
public void checkrc33data(int m,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3){   
...
public void checktdata(int i,int j,int k,float x,float y,float z){
...
public void check3Ddata(float x,float y,float z,float siz){if(siz==0){siz=size;}
...
public void checktr33_sampledata(){
...
public void point3Ddata(float x,float y,float z,float siz){if(siz==0){siz=size;}stroke(255*coordinateBW);
...
public void point3Dcvdata(float x,float y,float z,float siz){if(siz==0){siz=size;}stroke(255*coordinateBW);
...
public void point3D_RGBdata(float x,float y,float z,float R,float G,float B,float st,float siz){if(siz==0){siz=size;}
...
public void line3Ddata(float x,float y,float z,float w,float m,float n,float siz,float R,float G,float B,float strokeweight){if(siz==0){siz=size;}
...
public void Cuboiddata(float x0,float y0,float z0,float dr,int R,int G,int B,float siz,float strokeweight){if(siz==0){siz=size;}
...
public void spere3Ddata(float x,float y,float z,float R,float G,float B,float dr,float st,float siz){if(siz==0){siz=size;}stroke(255*coordinateBW);fill(R,G,B);
...
public void Bfill(float rangeminx, float rangemaxx, float rangeminy, float rangemaxy, float rangeminz, float rangemaxz) {
...
public void Bpoint3D(float x,float y,float z) {Bst.push(x, y, z);return;}

public void Bline3D(float x, float y, float z, float w, float m, float n) {  //구간마다 점 추가
    float dx,dy,dz; dx=(w-x)/1000;dy=(m-y)/1000;dz=(n-z)/1000;
    for (int i=0; i<=1000; i++) {Bpoint3D(x, y, z);x+=dx;y+=dy;z+=dz;}
    return;}

public void Bpointspe(float r,float angle1,float angle2) {
   float x,y,z;
...
public void Blinespe(float r1,float angle1,float angle2,float r2,float angle3,float angle4) {  //구간마다 점 추가
   float x,y,z,w,m,n;
...
public void Bpointcir(float r,float angle1,float z) {
   float x,y;
   x=r*cos(radians(angle1));
   y=r*sin(radians(angle1));
   Bst.push(x, y, z);return;}

public void Blinecir(float r1,float angle1,float z1,float r2,float angle2,float z2) {  //구간마다 점 추가
   float x,y,z,w,m,n;
   x=r1*cos(radians(angle1));y=r1*sin(radians(angle1)); z=z1;
   w=r2*cos(radians(angle2));m=r2*sin(radians(angle2));n=z2;
  float dx,dy,dz; dx=(w-x)/1000;dy=(m-y)/1000;dz=(n-z)/1000;
  for (int i=0; i<=1000; i++) {Bpoint3D(x, y, z);x+=dx;y+=dy;z+=dz;}return;}
public void point3D(float x,float y,float z,float siz){if(siz==0){siz=size;}stroke(255*coordinateBW);
  A[0]=x*cos(radians(ceta))-z*sin(radians(ceta));A[0]=A[0]*siz;
  A[1]=y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi));A[1]=A[1]*siz;
  strokeWeight(5);point(950+A[0],400+A[1]);fill(0);stroke(0);strokeWeight(5);}
  
public void point3D_RGB(float x,float y,float z,float R,float G,float B,float st,float siz){if(siz==0){siz=size;}
  A[0]=x*cos(radians(ceta))-z*sin(radians(ceta));A[0]=A[0]*siz;A[1]=y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi));A[1]=A[1]*siz;
  strokeWeight(st);stroke(R,G,B);point(950+A[0],400+A[1]);stroke(0);strokeWeight(5);}
  
public void line3D(float x,float y,float z,float w,float m,float n,float siz,float R,float G,float B,float strokeweight){if(siz==0){siz=size;}
  A[2]=x*cos(radians(ceta))-z*sin(radians(ceta));A[2]=A[2]*siz;
  A[3]=y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi));A[3]=A[3]*siz;
  A[4]=w*cos(radians(ceta))-n*sin(radians(ceta));A[4]=A[4]*siz;
  A[5]=m*cos(radians(pi))-w*sin(radians(pi))*sin(radians(ceta))-n*cos(radians(ceta))*sin(radians(pi));A[5]=A[5]*siz;
...
public void checkrc44(int m,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3,float x4,float y4,float z4){   
  S[m][0][0]=x1;S[m][0][1]=y1;S[m][0][2]=z1;
  S[m][1][0]=x2;S[m][1][1]=y2;S[m][1][2]=z2;
  S[m][2][0]=x3;S[m][2][1]=y3;S[m][2][2]=z3;
  S[m][3][0]=x4;S[m][3][1]=y4;S[m][3][2]=z4;}
  
public void checkrc33(int m,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3){   
  S[m][0][0]=x1;S[m][0][1]=y1;S[m][0][2]=z1;
  S[m][1][0]=x2;S[m][1][1]=y2;S[m][1][2]=z2;
  S[m][2][0]=x3;S[m][2][1]=y3;S[m][2][2]=z3;}
  
public void checkt(int i,int j,int k,float x,float y,float z){T[i][j][k][0]=x;T[i][j][k][1]=y;T[i][j][k][2]=z;}

public void check3D(float x,float y,float z,float siz){if(siz==0){siz=size;}
  A[0]=x*cos(radians(ceta))-z*sin(radians(ceta));A[0]=A[0]*siz;
  A[1]=y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi));A[1]=A[1]*siz;}
  
public void Cuboid(float x0,float y0,float z0,float dr,int R,int G,int B,float siz,float strokeweight){if(siz==0){siz=size;}point3D_RGB(x0,y0,z0,255,0,207,8,siz);
...
public void spere3D(float x,float y,float z,float R,float G,float B,float dr,float st,float siz){if(siz==0){siz=size;}stroke(255*coordinateBW);fill(R,G,B);
  A[0]=x*cos(radians(ceta))-z*sin(radians(ceta));A[0]=A[0]*siz;
  A[1]=y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi));A[1]=A[1]*siz;
  strokeWeight(st);ellipse(950+A[0],400+A[1],dr,dr);fill(0);stroke(0);strokeWeight(5);}
  
public void Bezier3D(float x0,float y0,float z0,float x1,float y1,float z1,float x2,float y2,float z2, float num,float siz){if(siz==0){siz=size;}
  strokeWeight(5);stroke(0xff08FF16);fill(255);textSize(20);float z,w,k,t=0;stroke(255);
  for(t=0;t<=1;t+=0.005f){z=t*t*x0+2*t*(1-t)*x1+(1-t)*(1-t)*x2;w=t*t*y0+2*t*(1-t)*y1+(1-t)*(1-t)*y2;k=t*t*z0+2*t*(1-t)*z1+(1-t)*(1-t)*z2;point3D_RGB(z,w,k,255,0,0,5,siz);}
...
public void Bezier_surface_control_line_rc44(float R,float G,float B,float siz){if(siz==0){siz=size;}
...
public void Bezier_surface_control_line_rc33(float R,float G,float B,float siz){if(siz==0){siz=size;}
...
public void Bezier_surface_line_rc44(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
...
public void Bezier_surface_line_rc33(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
  int i,j;float u,v;float[][][] save=new float[1005][1005][3];
   for(u=0;u<=1.001f;u+=gap){for(v=0;v<=1.001f;v+=gap){float sumx=0,sumy=0,sumz=0;
      for(i=0;i<3;i++){for(j=0;j<3;j++){sumx+=com[2][i]*com[2][j]*power(u,i)*power(v,j)*power(1-u,2-i)*power(1-v,2-j)*S[i][j][0];}}
      for(i=0;i<3;i++){for(j=0;j<3;j++){sumy+=com[2][i]*com[2][j]*power(u,i)*power(v,j)*power(1-u,2-i)*power(1-v,2-j)*S[i][j][1];}}
      ...
      save[(int)(u*1000)][(int)(v*1000)][0]=sumx;save[(int)(u*1000)][(int)(v*1000)][1]=sumy;save[(int)(u*1000)][(int)(v*1000)][2]=sumz;}}
   for(u=0;u<=1.001f;u+=gap){float savx=0,savy=0,savz=0;
    for(v=0;v<=1.001f;v+=gap){float sumx=save[(int)(u*1000)][(int)(v*1000)][0],sumy=save[(int)(u*1000)][(int)(v*1000)][1],sumz=save[(int)(u*1000)][(int)(v*1000)][2];
      point3D_RGB(sumx,sumy,sumz,R,G,B,5,siz);
      if(v==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3D(savx,savy,savz,sumx,sumy,sumz,siz,R,G,B,3);
        savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}
   for(v=0;v<=1.001f;v+=gap){float savx=0,savy=0,savz=0;
    for(u=0;u<=1.001f;u+=gap){float sumx=save[(int)(u*1000)][(int)(v*1000)][0],sumy=save[(int)(u*1000)][(int)(v*1000)][1],sumz=save[(int)(u*1000)][(int)(v*1000)][2];
      if(u==0){savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}
      else{line3D(savx,savy,savz,sumx,sumy,sumz,siz,R,G,B,3);
        savx=sumx;savy=sumy;savz=sumz;sumx=0;sumy=0;sumz=0;}}}}
        
public void Bezier_triangle_control_line44(float R,float G,float B,float siz){int i,j,k;if(siz==0){siz=size;}
...
public void Bezier_triangle_line44(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
  int i,j,k;float s,t,u,max=0;float[][][][] save=new float[105][105][105][3];
  for(s=0;s<=1;s+=gap){for(t=0;t<=1;t+=gap){if(s+t>1){continue;}u=1-s-t;float sumx=0,sumy=0,sumz=0;
        for(i=0;i<=3;i++){for(j=0;j<=3;j++){if(i+j<=3){k=3-i-j;sumx+=com2[i][j][k]*power(s,i)*power(t,j)*power(u,k)*T[i][j][k][0];}}}
        for(i=0;i<=3;i++){for(j=0;j<=3;j++){if(i+j<=3){k=3-i-j;sumy+=com2[i][j][k]*power(s,i)*power(t,j)*power(u,k)*T[i][j][k][1];}}}
...
public void Bezier_triangle_control_line33(float R,float G,float B,float siz){int i,j,k;if(siz==0){siz=size;}
...
public void Bezier_triangle_line33(float R,float G,float B,float siz,float gap){if(siz==0){siz=size;}
  int i,j,k;float s,t,u,max=0;float[][][][] save2=new float[105][105][105][3];
  for(s=0;s<=1;s+=gap){for(t=0;t<=1;t+=gap){if(s+t>1){continue;}u=1.01f-s-t;float sumx=0,sumy=0,sumz=0;
        for(i=0;i<=2;i++){for(j=0;j<=2;j++){if(i+j<=2){k=2-i-j;sumx+=com2[i][j][k]*power(s,i)*power(t,j)*power(u,k)*T[i][j][k][0];}}}
        for(i=0;i<=2;i++){for(j=0;j<=2;j++){if(i+j<=2){k=2-i-j;sumy+=com2[i][j][k]*power(s,i)*power(t,j)*power(u,k)*T[i][j][k][1];}}}
...
public void pointcir_RGB(float r,float angle1,float z,float R,float G,float B,float st,float siz){
   float x,y;
   x=r*cos(radians(angle1));
   y=r*sin(radians(angle1));
...
public void linecir(float r1,float angle1,float z1,float r2,float angle2,float z2,float siz,float R,float G,float B,float strokeweight){
  float x,y,z,w,m,n;
   x=r1*cos(radians(angle1));y=r1*sin(radians(angle1));z=z1;
   w=r2*cos(radians(angle2));m=r2*sin(radians(angle2));n=z2;
...
public void checkcir(float r,float angle1,float z,float siz){
   float x,y;
   x=r*cos(radians(angle1));y=r*sin(radians(angle1));
   A[0]=(x*cos(radians(ceta))-z*sin(radians(ceta)))*siz;
   A[1]=(y*cos(radians(pi))-x*sin(radians(pi))*sin(radians(ceta))-z*cos(radians(ceta))*sin(radians(pi)))*siz;}
   
public void pointspe_RGB(float r,float angle1,float angle2,float R,float G,float B,float st,float siz){
   float x,y,z;
   x=r*cos(radians(angle1))*cos(radians(angle2));
   y=r*cos(radians(angle1))*sin(radians(angle2));
...
public void linespe(float r1,float angle1,float angle2,float r2,float angle3,float angle4,float siz,float R,float G,float B,float strokeweight){
  float x,y,z,w,m,n;
   x=r1*cos(radians(angle1))*cos(radians(angle2));
   y=r1*cos(radians(angle1))*sin(radians(angle2));
   z=r1*sin(radians(angle1));
...
public void checkspe(float r,float angle1,float angle2,float siz){
   float x,y,z;
   x=r*cos(radians(angle1))*cos(radians(angle2));
   y=r*cos(radians(angle1))*sin(radians(angle2));
   z=r*sin(radians(angle1));
...
public void showstring(float x,float y){
...
public void istrue(){//println(str);
...
public void showstring2(float x,float y){
  ...
  for(int i=1;i<21;i++){strokeWeight(0.1f);
...
public void win4clear(){
  for(int i=0;i<win4length;i++){win4[i]="";}
  win4length=0;}

public void win4push(String t){
  win4[win4length]=t;
  win4length++;}
public void parseFile() {BufferedReader reader = createReader("data\\textfile\\sphere.obj");String line = null;String a=null;
      try {while ((line = reader.readLine()) != null) {String[] pieces=split(line,' ');
      if(pieces[0].equals("v")==true){obj[row][0]=Siz*PApplet.parseFloat(pieces[2]);obj[row][1]=Siz*PApplet.parseFloat(pieces[4]);obj[row][2]=Siz*PApplet.parseFloat(pieces[3]); //sphere obj[row][1],obj[row][2] change
      if(obj[row][2]-az<zmn){zmn=obj[row][2]-az;}if(obj[row][2]-az>zmx){zmx=obj[row][2]-az;}row++;}
      if(pieces[0].equals("vn")==true){vervec[rown][0]=10*PApplet.parseFloat(pieces[1]);vervec[rown][1]=10*PApplet.parseFloat(pieces[2]);vervec[rown][2]=10*PApplet.parseFloat(pieces[3]);rown++;}
      if(pieces[0].equals("f")==true){for(int i=1;i<5;i++){a=pieces[i];String[] pieces2=split(a,'/');//v vt vn
         index[rowf][i-1][0]=PApplet.parseInt(pieces2[0]);index[rowf][i-1][1]=PApplet.parseInt(pieces2[1]);index[rowf][i-1][2]=PApplet.parseInt(pieces2[2]);}
        rowf++;}}reader.close();} catch (IOException e) {e.printStackTrace();}}
        
public void parseFile2() {BufferedReader reader = createReader("data\\textfile\\heartmtl.mtl");String line = null;
    try {while ((line = reader.readLine()) != null) {String pieces2=trim(line);
      String[] pieces=split(pieces2,' ');//println(line);
      if(pieces[0].equals("Ns")==true){Ns=PApplet.parseFloat(pieces[1]);}
      if(pieces[0].equals("Ni")==true){Ni=PApplet.parseFloat(pieces[1]);}
      if(pieces[0].equals("d")==true){d=PApplet.parseFloat(pieces[1]);}
      if(pieces[0].equals("Tr")==true){Tr=PApplet.parseFloat(pieces[1]);}
      if(pieces[0].equals("illum")==true){illum=PApplet.parseFloat(pieces[1]);}
      if(pieces[0].equals("Tf")==true){Tf[0]=PApplet.parseFloat(pieces[1]);Tf[1]=PApplet.parseFloat(pieces[2]);Tf[2]=PApplet.parseFloat(pieces[3]);}
      if(pieces[0].equals("Ka")==true){Ka[0]=PApplet.parseFloat(pieces[1]);Ka[1]=PApplet.parseFloat(pieces[2]);Ka[2]=PApplet.parseFloat(pieces[3]);}
      if(pieces[0].equals("Kd")==true){Kd[0]=PApplet.parseFloat(pieces[1]);Kd[1]=PApplet.parseFloat(pieces[2]);Kd[2]=PApplet.parseFloat(pieces[3]);}
      if(pieces[0].equals("Ks")==true){Ks[0]=PApplet.parseFloat(pieces[1]);Ks[1]=PApplet.parseFloat(pieces[2]);Ks[2]=PApplet.parseFloat(pieces[3]);}
      if(pieces[0].equals("Ke")==true){Ke[0]=PApplet.parseFloat(pieces[1]);Ke[1]=PApplet.parseFloat(pieces[2]);Ke[2]=PApplet.parseFloat(pieces[3]);}
    }reader.close();} catch (IOException e) {e.printStackTrace();}}
    
public int parseFile3(){
...
public void rect3D_RGB(float x1,float y1,float z1,float R1,float G1,float B1,float x2,float y2,float z2,float R2,float G2,float B2,float x3,float y3,float z3,float R3,float G3,float B3,float x4,float y4,float z4,float R4,float G4,float B4,float R,float G,float B,float st,float siz){
   if(oneloop5==0)
...
public void ex1subfunction(){
     for(int i=0;i<10000;i++){p[i]=new vector();}
     for(int i=0; i<10; i++) for(int j=0; j<10; j++) {p[10*i+j]=new vector((2*i-9),(2*j-9),sqrt(200-(2*i-9)*(2*i-9)-(2*j-9)*(2*j-9)));u[10*i+j]=0.5f; v[10*i+j]=0.5f;}
...
public void showexample(){
      ...
      checkrc33(1,b[1][0].vectori(0),b[1][0].vectori(1),b[1][0].vectori(2),b[1][1].vectori(0),b[1][1].vectori(1),b[1][1].vectori(2),b[1][2].vectori(0),b[1][2].vectori(1),b[1][2].vectori(2));
      checkrc33(2,b[2][0].vectori(0),b[2][0].vectori(1),b[2][0].vectori(2),b[2][1].vectori(0),b[2][1].vectori(1),b[2][1].vectori(2),b[2][2].vectori(0),b[2][2].vectori(1),b[2][2].vectori(2));
      Bezier_surface_line_rc33(255, 0, 255, size, 0.05f);
...
public void setupcheck() {combination(20, 10);f(10);combination2();
    for(int i=0;i<10000;i++){win5[i]="";}
    for(int i=0;i<10000;i++){win5out[i]="";}
    for(int i=0;i<10;i++){for(int j=0;j<3;j++){rayshow[i][j]=0;}}
    Ka[0]=0.05f;Ka[1]=0.05f;Ka[2]=0.6f;}
    
public int f(int k) {if (k==1){fac[1]=1;return 1;}else{int s;s=k*f(k-1);fac[k]=s;return s;}}

public int combination(int a, int b) {if ((a==b)||(b==0)){com[0][0]=1;com[a][b]=1;return 1;}else {int s;s=combination(a-1, b-1)+combination(a-1, b);com[a][b]=s;return s;}}

public float power(float x, int n) {if (n==0) {return 1;}else {int i;float sum=1;for (i=0; i<n; i++){sum*=x;}return sum;}}
...
public void example_Bezier_rect(){ 
  ...
  Bezier_surface_line_rc33(255, 0, 0, size, 0.2f);
  Bezier_surface_control_line_rc33(237, 0, 255, size);}
  
public void example_Bezier_tri(){
  checktr33_sample();
  Bezier_triangle_line33(255, 0, 0, size, 0.2f);
  Bezier_triangle_control_line33(237, 0, 255, size);}
  
public void example_Bezier_line(){
  Bezier3D(100, 100, 100, -100, -100, -100, 50, 200, -150, 1, 0);}
  
public void example_Bezier_rect44(){ 
  ...
  Bezier_surface_line_rc44(255, 0, 0, size, 0.05f);
  Bezier_surface_control_line_rc44(255, 246, 142, size);}
  
public void example_Bezier_rect33(){ 
  ...
  Bezier_surface_line_rc33(255, 0, 0, size, 0.05f);
  Bezier_surface_control_line_rc33(255, 246, 142, size);}
  
public void example_Bezier_tri44(){
  checktr44();
  Bezier_triangle_line44(255, 0, 0, size, 0.1f);
  Bezier_triangle_control_line44(255, 246, 142, size);}
  
public void example_Bezier_tri33(){
  checktr33();
  Bezier_triangle_line33(0, 255, 0, size, 0.2f);
  Bezier_triangle_control_line33(255, 246, 142, size);}      
  
public void checktr44(){
...
public void ex6(){
  ...
  Bezier_surface_line_rc33RGB(size, 0.2f);
  //Bezier_surface_control_line_rc33(237, 0, 255, size);
}
public void Weighted_Bezier3D(float x0,float y0,float z0,float x1,float y1,float z1,float x2,float y2,float z2, float num,float siz,float w1R,float w1G,float w1B,float w2R,float w2G,float w2B,float w3R,float w3G,float w3B){if(siz==0){siz=size;}
  strokeWeight(5);stroke(0xff08FF16);fill(255);textSize(20);float z,w,k,t=0;stroke(255);
  for(t=0;t<=1;t+=0.005f){z=t*t*x0+2*t*(1-t)*x1+(1-t)*(1-t)*x2;w=t*t*y0+2*t*(1-t)*y1+(1-t)*(1-t)*y2;k=t*t*z0+2*t*(1-t)*z1+(1-t)*(1-t)*z2;
...
public void Bezier_surface_line_rc33RGB(float siz,float gap){if(siz==0){siz=size;}
  //fill(255);textSize(20);text("scale=sqrt(sqrt(k)*k)",413,70);
  int i,j;float u,v;float[][][] save=new float[1005][1005][3];float[][][] saveRGB=new float[1005][1005][3];
   for(u=0;u<=1.001f;u+=gap){for(v=0;v<=1.001f;v+=gap){float sumx=0,sumy=0,sumz=0;float sumR=0,sumG=0,sumB=0;
   ...
   for(u=0;u<=1.001f;u+=gap){float savx=0,savy=0,savz=0;float savR=0,savG=0,savB=0;
    for(v=0;v<=1.001f;v+=gap){
   ...
   for(v=0;v<=1.001f;v+=gap){float savx=0,savy=0,savz=0;float savR=0,savG=0,savB=0;
    for(u=0;u<=1.001f;u+=gap){
      float sumx=save[(int)(u*1000)][(int)(v*1000)][0],sumy=save[(int)(u*1000)][(int)(v*1000)][1],sumz=save[(int)(u*1000)][(int)(v*1000)][2];
      float sumR=saveRGB[(int)(u*1000)][(int)(v*1000)][0],sumG=saveRGB[(int)(u*1000)][(int)(v*1000)][1],sumB=saveRGB[(int)(u*1000)][(int)(v*1000)][2];
...
public void checkrc33RGB(int m,float x1,float y1,float z1,float x2,float y2,float z2,float x3,float y3,float z3,float w1R,float w1G,float w1B,float w2R,float w2G,float w2B,float w3R,float w3G,float w3B){   
  S[m][0][0]=x1;S[m][0][1]=y1;S[m][0][2]=z1;
...
public void weightedline3D(float x,float y,float z,float w,float m,float n,float siz,float R1,float G1,float B1,float R2,float G2,float B2,float strokeweight){if(siz==0){siz=size;}
